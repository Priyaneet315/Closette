...
},
{
  ...
  price: 150.00,
  description: "Elegant gold-tone watch with a clean minimalist dial.",
  tags: ["Minimal", "Accessories"],
...

{
  ...
  title: "Leather Belt",
  price: 45.00,
  description: "Premium leather belt with a sleek metal buckle.",
  tags: ["Workwear", "Accessories"],
  image: "https://images.unsplash.com/photo-1593032465171-8c3fbb1b59e0?w=600&q=80",
  sizes: [],
  avgRating: 4.4,
  isTopRated: false,
  type: "Accessories",
...

{
  ...
  price: 35.00,
  description: "Timeless silver hoops for everyday wear.",
  tags: ["Minimal", "Accessories"],
...

{
  ...
  price: 110.00,
  description: "Clean white sneakers that go with any outfit.",
  tags: ["Casual", "Accessories"],
  image: "https://images.unsplash.com/photo-1528701800489-20be9c5b8f2f?w=600&q=80",
  sizes: ["S", "M", "L", "XL"],
  avgRating: 4.7,
  isTopRated: true,
  type: "Accessories",
...
// Load
const savedWishlist = localStorage.getItem('closetteWishlist');
if (savedWishlist) {
    wishlist = JSON.parse(savedWishlist);
}
// Save function
function saveWishlist() {
    localStorage.setItem('closetteWishlist', JSON.stringify(wishlist));
}
...
function toggleWishlist(productId) {
    ...
    if (index === -1) {
        ...
        showToast(`${product.title} added to your wishlist!`, 3000);
    } else {
        wishlist.splice(index, 1);
        showToast(`${product.title} removed from your wishlist.`, 3000);
    }
...
// Delete a review by productId and username
function deleteReview(productId, username) {
    ...
    const reviewIndex = product.reviews.findIndex(r => r.user === username);
    if (reviewIndex === -1) {
        showToast("Review not found.", 3000);
        return;
    }

    // Remove the review
    const removed = product.reviews.splice(reviewIndex, 1)[0];
    showToast(`Your review for "${product.title}" was deleted.`, 3000);

    // Optionally, re-render the product modal or list
    renderProductReviews(productId);
}

function renderProductReviews(productId) {
    const product = products.find(p => p.id === productId);
    if (!product || !reviewsList) return;

    reviewsList.innerHTML = product.reviews.map((review, index) => {
        let deleteButtonHTML = '';
        if (review.user === localStorage.getItem('username')) {
            deleteButtonHTML = `<button class="delete-review-btn" onclick="deleteReview(${productId}, ${index})">Delete</button>`;
        }
        return `
            <div class="review-item">
                <strong>${review.user}</strong> - ${getStarRating(review.rating)}
                <p>${review.comment}</p>
                ${deleteButtonHTML}
            </div>
        `;
...
function renderCheckout() {
    ...
    const shippingValue = document.getElementById("shippingMethod")?.value || "standard";
    const shippingFee = shippingValue === "express" ? 15.00 : 5.00;
    ...
    contentHTML = `
        <form id="reviewForm" class="checkout-form">
            ...
            <fieldset style="border:1px solid #ddd; border-radius:8px; padding:15px; margin-bottom:20px;">
                ...
                <input type="text" value="${document.getElementById('address1')?.value || '123 Main St'}, ${document.getElementById('city')?.value || 'Anytown'}, ${document.getElementById('zip')?.value || '10001'}" readonly>
                
                <label>Payment Method</label>
                <input type="text" value="Card ending in ${document.getElementById('cardNumber')?.value.slice(-4) || '****'}" readonly>
            </fieldset>

            <div class="modal-actions" style="justify-content: space-between;">
                <button type="button" class="secondary-btn" onclick="checkoutStep = 2; renderCheckout();">Back to Payment</button>
                <button type="submit" class="primary-btn">Place Order ($${total.toFixed(2)})</button>
            </div>
        </form>
    `;
    ...
    document.getElementById('reviewForm').onsubmit = (e) => {
        e.preventDefault();
        const newOrder = {
            id: "O" + (mockOrders.length > 0 
                        ? (Math.max(...mockOrders.map(o => parseInt(o.id.substring(1)))) + 1) 
                        : 1001),
...
function renderSuggestedProductsInModal(currentProductId) {
    ...
    if (topRatedProducts.length === 0) {
        modalSuggestedSection.innerHTML = "<p style='padding: 10px 0; color: #666;'>No Top Rated Accessories available right now.</p>";
        return;
    }

    let html = `<h3>Top Rated Accessories</h3>
...
function renderProductDetail(productId) {
    ...
    const sizeOptionsHTML = product.sizes.map(size => {
        ...
        const recommendedTag = isDefault ? `(My Fit)` : '';
...
` : ''}
    ...
    <button id="wishlistBtn" class="secondary-btn">♡ Wishlist</button>
...
if (reviewBtn) {
    reviewBtn.addEventListener("click", () => {
        ...
        if (!rating || !comment) {
            showToast("Please select rating and write a review.", 3000);
            return;
        }
        ...
        product.avgRating = parseFloat((total / product.reviews.length).toFixed(1));

        showToast("Review submitted!", 2000);
    ...
    document.getElementById('addToCartBtn').onclick = () => {
       if (product.type !== "Accessories" && !selectedSize) {
    ...
    return;
        ...
        updateCartCount();
        showToast(
  `Added ${product.title}${selectedSize ? ` (${selectedSize})` : ''} to cart!`,
  3000
);
  ...
  const sizeGuideBtn = document.getElementById('viewSizeGuideBtn');
...
function renderOutfits() {
    ...
    if (topsForUser.length > 0 && bottomsForUser.length > 0) {
        ...
        for (let i = 0; i < Math.min(3, topsForUser.length, bottomsForUser.length); i++) {
            ...
            outfitsHTML += `
               <div class="wardrobe-item" data-id="${top.id}">
    ...
    const card = e.target.closest('.wardrobe-item');
    if (!card) return;
